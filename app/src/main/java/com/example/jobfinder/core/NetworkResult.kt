...
sealed class NetworkResult<out T> {
    data class Success<out T>(val data: T) : NetworkResult<T>() //chứa data trả về từ API
    data class Error(val message: String) : NetworkResult<Nothing>() //chứa thông báo lỗi, áp dụng cho nhiều trường hợp (mạng, server, exception,…)
}
